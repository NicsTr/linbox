...
bool test_set(BlasVector<Givaro::ZRing<Integer>>& X2, BlasMatrix<Givaro::ZRing<Integer>>& A, BlasVector<Givaro::ZRing<Integer>>& B
#ifdef __LINBOX_HAVE_MPI
              ,
              Communicator* Cptr
#endif
...
//	A.write(std::cout << " >>>> Compute with A:=",Tag::FileFormat::Maple) << ';' << std::endl;
        ...
        */
            ...
            MPI_Barrier(MPI_COMM_WORLD);
            endtime   = MPI_Wtime();
            std::cout<<"In Proc("<<Cptr->rank()<<") MPI data distribution used CPU time (seconds): "
